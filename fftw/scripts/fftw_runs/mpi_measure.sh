...
do
  ...
  do
    ...
    wisdomfile="${wisdir}/fftw_${arg}_p64_t${thread}_b${batch}.measure"
    echo "Writing to file : ${outfile}"

    #srun ../build/hybrid_many --num=$arg -i ${iter} -t ${thread} --batch=${batch} --wisdomfile=${wisdomfile} > ${outfile}
    mpirun --map-by ppr:16:socket --bind-to socket --report-bindings ../build_measure/hybrid_many --num=$arg -i ${iter} -t ${thread} --batch=${batch} --wisdomfile=${wisdomfile} > ${outfile}
  done
done
