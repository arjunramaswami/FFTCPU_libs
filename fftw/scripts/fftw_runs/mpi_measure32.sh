...
outdir="../data/measure/mpi2nodes/result"
...
do
  ...
  do
    ...
    wisdomfile="${wisdir}/fftw_${arg}_p32_t${thread}_b${batch}.measure"
    echo "Writing to file : ${outfile}"

    #srun ../build/hybrid_many --num=$arg -i ${iter} -t ${thread} --batch=${batch} --wisdomfile=${wisdomfile} > ${outfile}
    mpirun --map-by ppr:8:socket --bind-to socket --report-bindings ../build_measure/hybrid_many --num=$arg -i ${iter} -t ${thread} --batch=${batch} --wisdomfile=${wisdomfile} > ${outfile}
  done
done
